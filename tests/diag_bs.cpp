...
int main() {
    ...
    for (char c : keys) {
        ...
        std::cout << "Key: " << c << " -> BS: " << (int)res.backspace << ", Res: '"
                  << res.to_string() << "' (len: " << unicode::to_utf32(res.to_string()).size()
                  << ")\n";
    }
    ...
    std::cout << "Key: BACKSPACE -> BS: " << (int)res_bs.backspace
              << ", Action: " << (int)res_bs.action << "\n";

...
void test_rhymes_exhaustive() {
    ...
    std::cout << "DEBUG ngoèo: initial='" << s_check.initial << "', glide="
              << (s_check.glide.has_value() ? std::string(1, s_check.glide.value()) : "none")
              << ", vowel='" << s_check.vowel << "', final='" << s_check.final_c << "'"
              << std::endl;

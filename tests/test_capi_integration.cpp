 ...
 */
void assert_capi_typing(lotus_core_t* engine, const std::string& input,
                        const std::string& expected) {
    lotus_core_reset(engine);
    lotus_modifiers_t mods = {false, false};
    lotus_result_t res;
    for (char c : input) {
        res = lotus_core_process_key(engine, (char32_t)c, mods);
    }
...
void test_capi_integration() {
    ...
    lotus_core_set_tone_style(engine, LOTUS_TONE_NEW);
    assert_capi_typing(engine, "hoaf", "hoà");

    // 2. VNI Input
    lotus_core_set_method(engine, LOTUS_METHOD_VNI);

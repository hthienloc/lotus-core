...
pub struct LotusEngine {
    engine: *mut sys::lotus_core_t,
}

impl LotusEngine {
    /// Create a new Lotus Core instance.
    pub fn new() -> Self {
        let engine = unsafe { sys::lotus_core_create() };
        assert!(!engine.is_null(), "Failed to create Lotus Core instance");
        Self { engine }
    }
    ...
    pub fn process_key(&mut self, key: u32, shift: bool, caps_lock: bool) -> EngineResult {
        ...
        let res = unsafe { sys::lotus_core_process_key(self.engine, key, mods) };
    ...
    pub fn reset(&mut self) {
        unsafe { sys::lotus_core_reset(self.engine) };
    }
    ...
    pub fn set_method(&mut self, method: Method) {
        ...
        unsafe { sys::lotus_core_set_method(self.engine, c_method) };
    }
    ...
    pub fn set_tone_style(&mut self, style: ToneStyle) {
        ...
        unsafe { sys::lotus_core_set_tone_style(self.engine, c_style) };
    }
    ...
    pub fn set_free_w(&mut self, option: FreeW) {
        ...
        unsafe { sys::lotus_core_set_free_w(self.engine, c_option) };
    }

    /// Configure the manual hook keys option.
    pub fn set_std_uo(&mut self, enabled: bool) {
        unsafe { sys::lotus_core_set_std_uo(self.engine, enabled) };
    }
    ...
    pub fn add_shortcut(&mut self, trigger: &str, replacement: &str) {
        ...
        unsafe {
            sys::lotus_core_add_shortcut(self.engine, c_trigger.as_ptr(), c_replacement.as_ptr());
        }
    }

    /// Enables or disables automatic English word restoration.
    pub fn set_auto_restore(&mut self, enabled: bool) {
        unsafe { sys::lotus_core_set_auto_restore(self.engine, enabled) };
    }

    /// Enables or disables allowing non-standard initial consonants.
    pub fn set_allow_non_standard_initials(&mut self, enabled: bool) {
        unsafe { sys::lotus_core_set_allow_non_standard_initials(self.engine, enabled) };
    }
}

impl Drop for LotusEngine {
    fn drop(&mut self) {
        unsafe { sys::lotus_core_destroy(self.engine) };
    }
}

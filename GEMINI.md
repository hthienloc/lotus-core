...
- **Comment Preservation**: NEVER arbitrarily delete or strip comments (Doxygen or internal notes) unless they are directly invalidated by the code change. Maintaining context and documentation is a priority.
...
3. **Verify**: Run `./dev.sh` (tests) and `./dev.sh bench` (performance).
4. **Debug Log**: When using `tui_demo`, use `VNDEBUG=1` to see the aligned ASCII table log. Maintain the fixed-width formatting using `unicode::display_width`.

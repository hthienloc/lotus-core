...
void lotus_engine_add_shortcut(lotus_engine_t* engine, const char* trigger, const char* replacement) {
    if (!engine || !trigger || !replacement) return;
...
void lotus_engine_set_log_callback(lotus_log_callback_t callback) {
    if (!callback) {
        ...
        lotus_engine::set_log_callback([callback](lotus_engine::LogLevel level, const std::string& message) {
            lotus_log_level_t c_level;
            if (level == lotus_engine::LogLevel::ERROR) c_level = LOTUS_LOG_LEVEL_ERROR;
            else if (level == lotus_engine::LogLevel::INFO) c_level = LOTUS_LOG_LEVEL_INFO;
            else c_level = LOTUS_LOG_LEVEL_DEBUG;

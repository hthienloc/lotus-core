...
constexpr std::array<std::string_view, 28> VALID_INITIALS = {
    ...
    "ch", "gh", "gi", "kh", "ng", "nh", "ph", "qu", "th", "tr", "ngh"
};

constexpr std::array<std::string_view, 2> VALID_GLIDES = {
    "o", "u"
};
...
constexpr std::array<std::string_view, 12> VALID_FINALS = {
...
bool Validator::is_valid(const Syllable& syllable) {
    if (syllable.vowel.empty() && !syllable.glide.has_value()) return false;

    // 1. Component Set Checks
    if (!syllable.initial.empty()) {
        auto lower_i = unicode::to_lower(syllable.initial);
        if (std::find(VALID_INITIALS.begin(), VALID_INITIALS.end(), lower_i) == VALID_INITIALS.end()) return false;
    }

    if (syllable.glide.has_value()) {
        std::string g(1, (char)unicode::to_lower((char32_t)syllable.glide.value()));
        if (std::find(VALID_GLIDES.begin(), VALID_GLIDES.end(), g) == VALID_GLIDES.end()) return false;
    }
    ...
    if (!syllable.vowel.empty()) {
        ...
        if (std::find(VALID_NUCLEI.begin(), VALID_NUCLEI.end(), stripped_v) == VALID_NUCLEI.end()) return false;
    }

    if (!syllable.final_c.empty()) {
        std::string lower_f = syllable.final_c;
        if (std::find(VALID_FINALS.begin(), VALID_FINALS.end(), lower_f) == VALID_FINALS.end()) return false;
    }
    ...
    if (unicode::to_lower(syllable.initial) == "q") {
        if (!syllable.glide.has_value() || unicode::to_lower((char32_t)syllable.glide.value()) != 'u') return false;
    }
    ...
    if (syllable.glide.has_value()) affinity_char = unicode::to_lower((char32_t)syllable.glide.value());
    ...
    if (lower_init == "k" && !is_front) return false;
    if (lower_init == "c" && is_front) return false;
    if (lower_init == "gh" && !is_front_no_y) return false;
    if (lower_init == "g" && is_front_no_y) return false;
    if (lower_init == "ngh" && !is_front_no_y) return false;
    if (lower_init == "ng" && is_front_no_y) return false;

    // Diphthong vs Open/Closed Syllable (ia/iê, ua/uô, ưa/ươ)
    bool has_coda = !syllable.final_c.empty();
    if (syllable.vowel == "ia" && has_coda) return false; 
    if (syllable.vowel == "iê" && !has_coda) return false; 
    ...
    if (syllable.final_c == "ch" || syllable.final_c == "nh") {
        if (nucleus_start != 'a' && nucleus_start != U'ê' && nucleus_start != 'i' && nucleus_start != 'y') return false; 
    }
